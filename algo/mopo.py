...
class MOPO:
    ...
    def learn_dynamics(self):
        ...
        i = 0 # record the for-loop number
        while not break_training:
            for train_data_batch in dict_batch_generator(train_data, self.model_batch_size):
                model_log_infos = self.dynamics_model.update(train_data_batch, update_valnet)
                model_train_iters += 1
                self.model_tot_train_timesteps += 1
                if model_log_infos['loss/train_model_loss_val'] < 2. and update_valnet:
                    update_valnet = False
                
            # i += 1
            # update_valnet = False if i >= 1 else True
